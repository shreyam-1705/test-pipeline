...
bronze_query = parsed_df.writeStream \
    .format("delta") \
    .option("checkpointLocation", f"s3a://{bucket_name}/checkpoints/bronze") \
    .start(bronze_path)
...
silver_query = silver_df.writeStream \
    .format("delta") \
    .option("checkpointLocation", f"s3a://{bucket_name}/checkpoints/silver") \
    .start(silver_path)
